...
页面使用代码内 SVG 绘制四麦阵列，不依赖外部图片。显示层将算法坐标和麦克风标号整体顺时针旋转 90°，使 0° 向下、90° 向右、180° 向上、270° 向左；页面上 mic1 位于左下、mic2 右下、mic3 右上、mic4 左上。底层通道映射与物理坐标不变。原始角和跟踪角分别以不同颜色显示，并保留最近 10 秒跟踪轨迹。
...
- 页面契约测试验证 0°/90° 标签位置、指针公式与旋转后的麦克风标号；
