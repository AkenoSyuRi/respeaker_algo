...
pub fn run_record(opts: &RecordOptions) -> Result<(), String> {
    ...
    let (handle, actual_rate, actual_ch): (CaptureHandle, u32, u16) = match backend {
        Backend::Cpal => {
            ...
            let ch = cfg.channels();
            println!("输入设备: {name}");
            println!("流配置: {rate} Hz / {ch} 通道 / {:?}（cpal 共享模式）", cfg.sample_format());
    ...
    std::fs::create_dir_all(out_dir)
        .map_err(|e| format!("创建输出目录 {out_dir} 失败: {e}"))?;
    ...
    let deadline = (opts.duration > 0)
        .then(|| Instant::now() + Duration::from_secs(opts.duration));
    let started = Instant::now();
    let frames = if actual_ch == 6 {
        println!(
            "开始录制 ->\n  {algo_path}  (ch0 算法输出)\n  {mic_path}  (ch1-4 麦克风原始)\n  {ref_path}  (ch5 回放)\nCtrl+C 或 {} 秒后停止",
            if opts.duration > 0 { opts.duration.to_string() } else { "不限时".to_string() }
        );
        write_split_loop(&algo_path, &mic_path, &ref_path, actual_rate, &rx, &stop, deadline)?
    } else {
...
/// 把 6 通道交织样本拆分为三路：algo（ch0）、mic（ch1-4，4ch）、ref（ch5）。
fn split_6ch(samples: &[i16]) -> (Vec<i16>, Vec<i16>, Vec<i16>) {
    let n = samples.len() / 6;
    let mut algo = Vec::with_capacity(n);
    let mut mic = Vec::with_capacity(n * 4);
...
fn write_split_loop(
    ...
    let mut algo = WavSink::create(algo_path, 1, rate)?;
    let mut mic = WavSink::create(mic_path, 4, rate)?;
    let mut rf = WavSink::create(ref_path, 1, rate)?;
    ...
    loop {
        ...
        match rx.recv_timeout(Duration::from_millis(100)) {
            Ok(samples) => {
                let (a, m, r) = split_6ch(&samples);
                algo.write_samples(&a)
                    .map_err(|e| format!("写入 {} 失败: {e}", algo_path))?;
                mic.write_samples(&m)
                    .map_err(|e| format!("写入 {} 失败: {e}", mic_path))?;
                rf.write_samples(&r)
                    .map_err(|e| format!("写入 {} 失败: {e}", ref_path))?;
    ...
    algo.finalize().map_err(|e| format!("完成 {} 失败: {e}", algo_path))?;
    mic.finalize().map_err(|e| format!("完成 {} 失败: {e}", mic_path))?;
    rf.finalize().map_err(|e| format!("完成 {} 失败: {e}", ref_path))?;
    Ok(frames)
}
...
mod tests {
    ...
    fn split_6ch_works() {
        ...
        let (algo, mic, rf) = split_6ch(&samples);
    ...
    fn default_prefix_format() {
        ...
        assert_eq!(p.len(), 15);
        assert_eq!(&p[8..9], "_");
        assert!(p.chars().enumerate().all(|(i, c)| {
            c.is_ascii_digit() || (i == 8 && c == '_')
        }));
    }
}
